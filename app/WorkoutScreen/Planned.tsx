...
const WorkoutTemplatesTab = () => {
 ...
 const fetchPlanned = async (userId: string) => {
  try {
    setLoading(true);
    const response = await axios.get(`/workouts/planned/${userId}`);
    const parsedData = typeof response.data === 'string' ? JSON.parse(response.data) : response.data;
    setTemplates(parsedData.filter((workout: WorkoutTemplate) => workout.status === 'planned'));
  } catch (err) {
  ...
  useFocusEffect(
    React.useCallback(() => {
      if (userId) {
        fetchPlanned(userId); // Fetch templates whenever the screen comes into focus
      }
    ...
    >
      <View style={styles.cardHeader}>
        ...
        <Text style={styles.workoutDate}>
          {new Date(item.status === 'planned' ? item.plannedDate : item.status).toLocaleDateString('en-US', {
            month: 'short',
            day: 'numeric',
            year: 'numeric'
          })}
