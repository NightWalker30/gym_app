...
const fetchLogs = async (userId: string) => {
  try {
    ...
    setTemplates(parsedData.filter((workout: WorkoutTemplate) => workout.status === 'completed'));
  } catch (err) {
  ...
  useFocusEffect(
    React.useCallback(() => {
      if (userId) {
        fetchLogs(userId); // Fetch templates whenever the screen comes into focus
      }
    ...
    >
      <View style={styles.cardHeader}>
        ...
        <Text style={styles.workoutDate}>
          {new Date(item.status === 'completed' ? item.completedDate : item.status).toLocaleDateString('en-US', {
            month: 'short',
            day: 'numeric',
            year: 'numeric'
          })}
